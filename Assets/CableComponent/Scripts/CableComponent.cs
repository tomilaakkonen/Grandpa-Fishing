...
public class CableComponent : MonoBehaviour
{
    ...
    {
        GetComponent<SpringJoint>().maxDistance -= reelSpeed;
    }

    public void ButtonOut()
    {
        GetComponent<SpringJoint>().maxDistance += throwDistance;
    }
...
void RenderCable()
	...
	void FixedUpdate()
	{
        ...
        if (GetComponent<SpringJoint>().maxDistance <= cableLength)
        {
            GetComponent<SpringJoint>().maxDistance = cableLength;
        }

        if (GetComponent<SpringJoint>().maxDistance >= cableMaxLength)
        {
            GetComponent<SpringJoint>().maxDistance = cableMaxLength;
        }
        if (GameObject.Find("Cable_start").gameObject.tag == "Hooked")
        {
            reelSpeed = 0.07f;
        }
        if (GameObject.Find("Cable_start").gameObject.tag == "Hook")
        {
            reelSpeed = 0.14f;
        }
        //#endif
